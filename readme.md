...
# Concurrency
위와 같은 구조에서 한가지 문제점이 발생합니다.
예를 들어, A라는 audio의 mp3 파일을 생성하고 있을 때, A audio의 텍스트를 수정했다고 가정해봅시다.
REPEATABLE READ level에서는 각각의 트랜잭션은 독립된 스냅샷을 가지고 작업을 수행하고, 서로 다른 transaction이 서로에게 영향을 주는 것을 허용하지 않습니다.
mp3파일을 생성하는 작업 중간에, 해당 audio의 텍스트가 수정된다면 race condition이 발생합니다.
![](https://velog.velcdn.com/images/yomapi/post/e1607df8-7d20-457d-9113-af47856d3cd2/image.png)

scheduler의 mp3 생성 job이 select 할 때 select_for_update를 사용하여, exclusive lock을 걸어줍니다.
skip locked 옵션을 사용하여, 이미 lock이 걸린경우 기다리지 않고, mp3 생성이 필요한 다른 audio를 select합니다.

> 💡고려한 점을 보다 자세히 알고 싶다면 제 [블로그 글](https://velog.io/@yomapi/series/%EA%B5%AC%EC%A1%B0%EA%B0%9C%EC%84%A0-transaction-%EC%82%AC%EC%9A%A9%ED%95%98%EA%B8%B0)을 참고해주세요 :)
